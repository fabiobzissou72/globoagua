...
const LeafletMap = dynamic(() => import('../../admin/rastreamento/MapComponent'), {
  ssr: false,
  loading: () => (
...
export default function SuperAdminRastreamentoPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`super-admin-driver-locs-${Math.random()}`)
    ...
    <div className="flex flex-col md:flex-row md:h-full">
      ...
      <aside className="bg-white border-b md:border-b-0 md:border-r flex-shrink-0 md:w-64 flex flex-col">
        <div className="px-4 py-3 border-b">
          <h2 className="font-bold text-gray-900 text-sm">Rastreamento GPS</h2>
          <p className="text-xs text-gray-500">{drivers.length} entregadores</p>
        </div>
