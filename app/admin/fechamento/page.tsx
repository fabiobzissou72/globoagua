...
type Driver = { id: string; nome_completo?: string }
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={exportCSV} className="btn-primary py-2 px-4 text-sm">
      ...
      <div className="card p-4 mb-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="input-base py-2 text-sm" />
        <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="input-base py-2 text-sm" />
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
        {[
          ...
          { label: 'PIX', value: formatCurrency(orders.filter(o => o.metodo_pagamento === 'pix').reduce((s, o) => s + o.total, 0)), color: '#2E7D32' },
          { label: 'Dinheiro', value: formatCurrency(orders.filter(o => o.metodo_pagamento === 'dinheiro').reduce((s, o) => s + o.total, 0)), color: '#F57C00' },
        ].map(card => (
