...
export default function DesempenhoPage() {
  ...
  async function loadStats() {
    ...
    driversData.forEach(d => {
      statsMap[d.id] = {
        driver_id: d.id,
        driver_name: d.nome_completo || d.email || 'Entregador',
        entregas: 0,
        cancelamentos: 0,
        total_pedidos: 0,
        taxa: 0,
    ...
    ordersData.forEach(order => {
      ...
      if (!statsMap[order.driver_id]) return
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="flex flex-wrap gap-3 mb-5 items-center">
        {[
          { label: 'Hoje', range: 'hoje' as const },
          { label: 'Esta semana', range: 'semana' as const },
          { label: 'Este mês', range: 'mes' as const },
        ].map(({ label, range }) => (
                ...
                {stats.map(driver => (
                  <tr key={driver.driver_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <span className="font-bold text-[#2E7D32]">{driver.entregas}</span>
                    </td>
                    <td className="px-4 py-3">
